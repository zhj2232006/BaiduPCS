...
static inline char* pcs_http_perform(struct pcs_http* http, const char* url)
{
    ...
    if (res != CURLE_OK) {
        if (!http->strerror) {
            http->strerror = pcs_utils_sprintf("%s: %s", curl_easy_strerror(res), url);    //pcs_utils_strdup(curl_easy_strerror(res));
        }
        return NULL;
    }
                                      ...
                                      const char* filename, const char* simulate_filename)
{
    ...
    if (escape_simulate_filename) {
        curl_free((void*)escape_simulate_filename);
    }
    return res;
}
